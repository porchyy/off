...
## โหมดตัวอย่าง (static, ไม่มี backend)

ถ้าเปิด `frontend/dist/index.html` ผ่าน static server (หรือ deploy ไป
GitHub Pages) แอปจะตรวจ `GET /api/health` แล้วสลับเป็น localStorage
อัตโนมัติ — ฟีเจอร์กล้อง, จับท่าทาง, คำนวณคะแนน, แจ้งเตือนทำงานเหมือนกัน
ต่างกันแค่ที่เก็บข้อมูล
