...
import { createStorage, defaultSettings, BackendUnavailableError } from './storage.js';
...
async function connectToBackend() {
  ...
  try {
    ...
    start.disabled = running; // enable "start" unless already running
    return true;
  } catch (err) {
...
async function checkHealth() {
  ...
  try {
    ...
    el.textContent = 'เชื่อมต่อ backend ไม่ได้';
...
$('retryBackend').addEventListener('click', async () => {
  if (await connectToBackend()) {
    await loadSettings();
    await checkHealth();
    await loadDashboard();
    toast('เชื่อมต่อ backend สำเร็จแล้ว');
  }
});
$('clearData').addEventListener('click', async () => {
  const ok = confirm('ลบประวัติและแจ้งเตือนทั้งหมดจากฐานข้อมูลในเครื่อง?');
  if (!ok) return;
  await storage.clear();
  await loadDashboard();
  toast('ลบข้อมูลแล้ว');
